...
public class Modalidade extends JPanel {
	...
	private JTextField inputNome = new JTextField(30);
	private JTextField inputValor = new JTextField(30);;
	...
	private void organizarEventos() {
		salvar.addActionListener(
				(event) -> {
					...
					ModalidadeDTO modalidade = new ModalidadeDTO(nome, valor);
					...
					for(int i = 0; i < listaModalidades.getSize(); i++) {
						Object[] lista = {
								listaModalidades.getOne(i).getNome(),
								listaModalidades.getOne(i).getValor()
						};
		...
		selecionarLinha.addActionListener(
				(event) -> {
